...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
   ...
   /* @Autowired
    ...
    public ConfirmOrderResult generateConfirmOrder() {
        ...
        List<CartPromotionItem> cartPromotionItemList = cartItemService.listPromotion(ilonwUserBTO.getIlonwUserId());
    ...
    public CommonResult generateOrder(OrderParam orderParam) {
        ...
        //获取购物车及优惠信息 todo
        IlonwUserBTO currentMember = null;
        List<CartPromotionItem> cartPromotionItemList = cartItemService.listPromotion(currentMember.getIlonwUserId());
        ...
        //转化为订单信息并插入数据库
        order.setMemberId(currentMember.getIlonwUserId());
        order.setCreateTime(new Date());
        order.setMemberUsername(currentMember.getIlonwUserNikename());
        ...
        //生成订单号
        order.setOrderSn(UUIDUtil.getOrderIdByUUId());
        ...
        //如使用优惠券更新优惠券使用状态
        if (orderParam.getCouponId() != null) {
            updateCouponStatus(orderParam.getCouponId(), currentMember.getIlonwUserId(), 1);
        }
        //如使用积分需要扣除积分 todo
        /*if (orderParam.getUseIntegration() != null) {
        ...
        //删除购物车中的下单商品
        deleteCartItemList(cartPromotionItemList, null);
     ...
     * 删除下单商品的购物车信息
     */
    private void deleteCartItemList(List<CartPromotionItem> cartPromotionItemList, IlonwUserBTO currentMember) {
        List<Long> ids = new ArrayList<>();
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            ids.add(cartPromotionItem.getId());
        }
        cartItemService.delete(currentMember.getIlonwUserId(), ids);
    }
     ...
     * @param ilonwUserBTO  使用的用户
     * @param hasCoupon      是否已经使用优惠券
     */
    private BigDecimal getUseIntegrationAmount(Integer useIntegration, BigDecimal totalAmount, IlonwUserBTO ilonwUserBTO, boolean hasCoupon) {
        BigDecimal zeroAmount = new BigDecimal(0);
        //判断用户是否有这么多积分 todo
        /*if (useIntegration.compareTo(ilonwUserBTO.getIlonwUserAge()) > 0) {
            return zeroAmount;
        }*/
        //根据积分使用规则判断是否可用
        //是否可与优惠券共用
